/*
	...
	Distances are measured from object outline to inline.
...
*/
	...
	*/
	...
	allInlinePxXs = newArray(imageWidth*imageHeight);
	allInlinePxYs = newArray(imageWidth*imageHeight);
	bgI = getPixel(0, 0);
	oPxls = 0;
	...
	for (oROI=0; oROI<rOIs; oROI++){
		...
		outlinePxXs = newArray(maxPoints);
		outlinePxYs = newArray(maxPoints);
		...
		for(iO=0; iO<rOIs; iO++){
			...
			else oCDSqs[iO] = (oROI+1)*1E50; /* just something very large to eliminate oROI from the sort */
		}
		...
		for (dROI=0; dROI<maxNNOs; dROI++){
			...
			minDSqs[dROI] = (dROI+1)*1E50; /* just something very large that changes with iteration for debugging purposes */
			for (rPx=0; rPx<rPxls; rPx++){ /* for all originating ROI outline pix */
				x1 = outlinePxXs[rPx];
				y1 = outlinePxYs[rPx];
				for (dROIpx=0; dROIpx<oPxlsF; dROIpx++){  /* for all inline perimeter pix */
